...
router.get('/members', auth, async (req, res) => {
  try {
    ...
    // Build the query - only exclude current user
    const query = {
      _id: { $ne: req.user.id }
    };
    ...
    // Map users with their connection status and skill matches
    const members = users.map(user => {
      ...
      // Calculate matching skills (skills that match user's interests)
    ...
    // Sort members by number of matching skills (descending)
    ...
    // Ensure we always send a valid JSON response even in case of error
